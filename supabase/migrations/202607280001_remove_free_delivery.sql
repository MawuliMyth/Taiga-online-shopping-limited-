...
begin
  if delivery_method not in ('standard','pickup') then raise exception 'Choose a valid delivery method'; end if;
  ...
  shipping:=case when delivery_method='pickup' then settings.pickup_shipping_fee else settings.standard_shipping_fee end;
...
set announcement_left='Nationwide delivery available'
where id=1 and announcement_left ilike '%free delivery%';
