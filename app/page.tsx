...
function whatsappUrl(phone:string){
  ...
  const message=encodeURIComponent("Hello Taiga, I need help with a product or order.");
  return `https://wa.me/${international}?text=${message}`;
}

type Product = { id: string; slug: string; name: string; category: string; price: number; old: number; rating: number; badge: string; image: string; variants?:any[]; createdAt:string; salesCount:number };
...
export default function Home() {
  ...
  const [storeSettings,setStoreSettings]=useState<any>({announcement_left:"Nationwide delivery available",announcement_center:"Football celebration: save up to 50%",announcement_right:"Call / WhatsApp: 0800 466 3639",flash_sale_title:"Flash Sales",support_phone:"0800 466 3639"});
  ...
  useSessionTimeout(60,()=>{setUser(null);setPanel(null);setNotice("Your session timed out after 60 minutes of inactivity. Please sign in again.")});
  const [navSection,setNavSection]=useState("deals");
  ...
  const newArrivals=useMemo(()=>[...liveProducts].sort((a,b)=>new Date(b.createdAt).getTime()-new Date(a.createdAt).getTime()).slice(0,8),[liveProducts]);
  const mostWanted=useMemo(()=>[...liveProducts].sort((a,b)=>b.salesCount-a.salesCount||b.rating-a.rating).slice(0,8),[liveProducts]);
  ...
  useEffect(() => {
    ...
    supabase.from("products").select("id,slug,name,price,compare_at_price,rating,badge,image_url,variants,created_at,sales_count,categories(name)").eq("is_active", true).order("created_at",{ascending:false}).then(({data,error}) => {
      if(error)setCatalogError("The catalogue could not be loaded. Please try again shortly.");
      else setLiveProducts((data??[]).map((p: any) => ({id:p.id,slug:p.slug,name:p.name,category:categoryAliases[p.categories?.name]??p.categories?.name??"Others",price:Number(p.price),old:Number(p.compare_at_price ?? p.price),rating:Number(p.rating),badge:p.badge ?? "New",image:p.image_url,variants:p.variants??[],createdAt:p.created_at,salesCount:Number(p.sales_count??0)})));
  ...
  useEffect(()=>{const observer=new IntersectionObserver(entries=>{const visible=entries.filter(entry=>entry.isIntersecting).sort((a,b)=>b.intersectionRatio-a.intersectionRatio)[0];if(visible)setNavSection(visible.target.id)},{rootMargin:"-140px 0px -55%",threshold:[.1,.35,.6]});["deals","categories","new","popular"].forEach(id=>{const element=document.getElementById(id);if(element)observer.observe(element)});return()=>observer.disconnect()},[catalogLoading,liveProducts.length]);
  ...
  async function addToCart(product: Product){
    ...
    if(result.error){setCart(current=>Math.max(0,current-1));return flash("Cart could not be updated yet.")} flash("Added to your cart");
  }
  ...
  return <div className="storefront">{showAuth&&<AuthModal reason={authReason} onClose={()=>setShowAuth(false)}/>} {panel&&user&&<StorePanels kind={panel} user={user} onClose={()=>setPanel(null)} onChanged={reloadCounts}/>} {notice&&<div className="toast">{notice}</div>}
    <div className="announcement"><span>{/free delivery/i.test(String(storeSettings.announcement_left||""))?"Nationwide delivery available":storeSettings.announcement_left||"Nationwide delivery available"}</span><span>{storeSettings.announcement_center}</span><span>Call / WhatsApp: {storeSettings.support_phone||"0800 466 3639"}</span></div>
    <header className="site-header">
      <div className="header-main wrap">
        <Link href="/" className="logo"><span>T</span>Taiga<small>MARKET</small></Link>
        <div className="search-shell"><form className="search" onSubmit={event=>{event.preventDefault();window.location.href=`/search?q=${encodeURIComponent(query.trim())}`}}><Search size={19} /><input value={query} onFocus={()=>setSearchOpen(true)} onChange={e => {setQuery(e.target.value);setSearchOpen(true)}} placeholder="Search products, brands and categories" /><button type="submit">Search</button></form>{searchOpen&&<div className="search-popover"><div className="search-popover-title"><span>{query?"Matching products":"Trending searches"}</span><button onClick={()=>setSearchOpen(false)}><X size={16}/></button></div>{query?<div className="search-results">{liveProducts.filter(p=>p.name.toLowerCase().includes(query.toLowerCase())).slice(0,5).map(p=><button key={p.id} onClick={()=>window.location.href=`/search?q=${encodeURIComponent(p.name)}`}><img src={p.image} alt=""/><span>{p.name}<small>{p.category}</small></span><strong>₦{p.price.toLocaleString()}</strong></button>)}</div>:<div className="trend-tags">{["headphones","air fryer","iphone","shoes for men","smart watch"].map(t=><button key={t} onClick={()=>window.location.href=`/search?q=${encodeURIComponent(t)}`}>{t}</button>)}</div>}</div>}</div>
        <div className="header-actions">
          ...
          <div className="account-menu-shell">
            ...
            {user&&accountMenuOpen&&<div className="account-dropdown" role="menu">
              <button role="menuitem" onClick={()=>window.location.href="/account/account"}><UserRound/><span>My Account<small>Profile and account details</small></span></button>
              <button role="menuitem" onClick={()=>window.location.href="/account/orders"}><ShoppingBag/><span>Orders<small>Track current purchases</small></span></button>
              <button role="menuitem" onClick={()=>window.location.href="/account/inbox"}><Mail/><span>Inbox<small>Order updates and notices</small></span></button>
              <button role="menuitem" onClick={()=>window.location.href="/account/wishlist"}><Heart/><span>Wishlist<small>{wishlist.size} saved {wishlist.size===1?"item":"items"}</small></span></button>
          ...
          <button className="icon-action" onClick={()=>window.location.href="/account/wishlist"} aria-label={`Open wishlist${wishlist.size>0?`, ${wishlist.size} saved item${wishlist.size===1?"":"s"}`:""}`}><Heart />{wishlist.size>0&&<b data-count={wishlist.size}>{wishlist.size}</b>}</button>
          <button className="icon-action cart-action" onClick={()=>window.location.href="/account/cart"} aria-label={`Open cart${cart>0?`, ${cart} item${cart===1?"":"s"}`:""}`}><ShoppingCart /><span>Cart</span>{cart>0&&<b data-count={cart}>{cart}</b>}</button>
        </div>
      </div>
      <nav className="nav wrap">
        ...
        <Link href="/shop">All products</Link><Link href="/shop?sort=deals">Today&apos;s deals</Link><Link href="/shop?sort=new">New arrivals</Link><Link href="/shop?sort=popular">Most wanted</Link>
        <button onClick={() => window.location.href="/account/orders"} className="admin-link" style={{ background: "transparent", border: "0", cursor: "pointer", fontSize: "inherit", fontWeight: "inherit", padding: "8px 0" }}>
          <ClipboardCheck size={16} /> Track Order
        </button>
      </nav>
      {departmentsOpen&&<div className="mega-menu wrap" onMouseLeave={()=>setDepartmentsOpen(false)}><aside>{catalogCategories.map(({name,icon:Icon})=><button className={hoveredDepartment===name?"active":""} key={name} onMouseEnter={()=>setHoveredDepartment(name)} onFocus={()=>setHoveredDepartment(name)} onClick={()=>window.location.href=`/shop?category=${encodeURIComponent(name)}`}><Icon size={17}/>{name}<ChevronRight size={14}/></button>)}</aside><div className="mega-detail"><div className="mega-detail-heading"><span>Explore</span><h3>{hoveredDepartment}</h3></div><div className="mega-columns">{(departmentDetails[hoveredDepartment]??[{title:hoveredDepartment,items:[`All ${hoveredDepartment}`,"New arrivals","Best sellers"]}]).map(group=><div key={group.title}><h4>{group.title}</h4>{group.items.map(item=><button key={item} onClick={()=>window.location.href=`/search?q=${encodeURIComponent(item)}&category=${encodeURIComponent(hoveredDepartment)}`}>{item}</button>)}</div>)}</div></div><div className="mega-support"><strong>Need help?</strong><span>Call / WhatsApp</span><b>{storeSettings.support_phone||"0800 466 3639"}</b><a href={`mailto:${storeSettings.support_email||"support@taiga.ng"}`}>Contact support</a></div></div>}
    </header>
    ...
    <main>
      ...
      <section className="benefits wrap" id="service"><Link href="/help/delivery"><Truck /><span><strong>Nationwide delivery</strong><small>Fees shown at checkout</small></span></Link><Link href="/help/returns"><PackageCheck /><span><strong>7-day returns</strong><small>Eligibility conditions apply</small></span></Link><Link href="/help/terms"><BadgeCheck /><span><strong>Secure payment</strong><small>Bank transfer or card</small></span></Link><a href={`tel:${String(storeSettings.support_phone||"").replace(/\s/g,"")}`}><Headphones /><span><strong>Call support</strong><small>{storeSettings.support_phone||"0800 466 3639"}</small></span></a></section>

      <section className="section wrap" id="categories"><div className="section-heading"><div><span className="kicker">Find your favorite</span><h2>Shop by category</h2></div><Link href="/shop">View all <ArrowRight size={16}/></Link></div><div className="category-grid">{catalogCategories.map(({name,image,icon:Icon,subcategories})=><article className={`category-item ${active===name?"active":""}`} key={name}><button className="category-main" onClick={()=>window.location.href=`/shop?category=${encodeURIComponent(name)}`}><div><img src={image} alt=""/><span><Icon size={17}/></span></div><strong>{name}</strong><small>{liveProducts.filter(p=>p.category===name).length} products</small><ChevronRight className="category-chevron" size={15}/></button><div className="subcategory-menu"><div><Icon size={18}/><strong>{name}</strong></div>{subcategories.map(sub=><button key={sub} onClick={()=>window.location.href=`/search?q=${encodeURIComponent(sub)}&category=${encodeURIComponent(name)}`}>{sub}<ChevronRight size={12}/></button>)}<button className="view-category" onClick={()=>window.location.href=`/shop?category=${encodeURIComponent(name)}`}>View all {name}</button></div></article>)}</div></section>
      ...
      {newArrivals.length>0&&<section className="section wrap catalogue-section" id="new"><div className="section-heading"><div><span className="kicker">Just added</span><h2>New arrivals</h2><p>The newest products published by Taiga.</p></div><a href="#deals">View full catalogue <ArrowRight size={16}/></a></div><div className="product-grid">{newArrivals.slice(0,4).map(product => <ProductCard key={product.id} product={product} onAdd={() => addToCart(product)} onLike={()=>toggleWishlist(product)} isLiked={wishlist.has(String(product.id))} />)}</div></section>}
      {mostWanted.length>0&&<section className="section wrap catalogue-section" id="popular"><div className="section-heading"><div><span className="kicker">Customer favourites</span><h2>Most wanted</h2><p>Ranked automatically by completed purchases and customer ratings.</p></div><a href="#deals">See everything <ArrowRight size={16}/></a></div><div className="product-grid">{mostWanted.slice(0,4).map(product => <ProductCard key={product.id} product={product} onAdd={() => addToCart(product)} onLike={()=>toggleWishlist(product)} isLiked={wishlist.has(String(product.id))} />)}</div></section>}
